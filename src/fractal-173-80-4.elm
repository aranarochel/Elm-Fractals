...
update model x = 
  ...
  in if (model.level == endLevel) then {init | seed = seed', offsetM = offset}
       ...
       { model | frame = nextFrame, offsetM = offset+((toFloat x)/10)
       }
