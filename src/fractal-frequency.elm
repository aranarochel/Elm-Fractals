...
init : Model
init = { points = [(-600.0, -10.0),(-400.0, -10.0), (0.0,-10),(200.0,-10), (50.0,-10),(400,-10)]
       , level = 0
       , frame = 0
       , seed = seed0
       , offsetM = 1.0
...
update model x = 
  ...
  in if (model.level == endLevel) then {init | seed = seed', offsetM = offset}
       ...
       , frame = 0
       , seed = seed'
       , offsetM = model.offsetM+((toFloat x)/10)
       }
       ...
       { model | frame = nextFrame, offsetM = model.offsetM+((toFloat x)/10)
       }
...
view (w,h) model =
  let offset = model.offsetM * toFloat (model.frame) / toFloat frameCount
      ...
      colors = [darkOrange, lightGray, darkBlue, darkYellow]
  in layers
...
main =
  (Signal.foldp (\_ c -> c+1) 0 (every (5*millisecond))
     |> Signal.sampleOn) (Signal.map .x Keyboard.arrows)
     |> Signal.foldp (\i d -> update d i) init 
